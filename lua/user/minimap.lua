  ...
  callback = function()
    if vim.bo.filetype == "minimap" then
      ...
      for _, win in ipairs(wins) do
        ...
        if ft ~= "minimap" and ft ~= "NvimTree" and ft ~= "trouble" then normal_wins = normal_wins + 1 end
      end
      if normal_wins == 0 then vim.cmd "MinimapClose" end
    end
  end,
})
...
function M.refresh_if_open()
  ...
  if #minimap_wins > 0 then vim.cmd "MinimapRefresh" end
end
...
vim.api.nvim_create_autocmd({ "BufWritePost", "TextChanged", "TextChangedI" }, {
  ...
  callback = function()
    ...
    vim.defer_fn(function() M.refresh_if_open() end, 100)
  end,
})
...
vim.api.nvim_create_user_command("MinimapTest", function()
  ...
  if result and result:match "%S" then
    ...
    if version and version:match "%S" then print("✓ Version: " .. version:gsub("\n", "")) end
  else
