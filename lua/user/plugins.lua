...
require("lazy").setup({
                ...
                win = {
                    keys = {
                        interrupt = {
                            "<C-c>",
                            function(terminal)
                                if terminal.job and terminal:is_running() then vim.api.nvim_chan_send(terminal.job,
                                        "\x03") end
                            end,
                            mode = "t",
    ...
    -- ============================================
    -- DESFLOW: DESFlow request manager
    -- ============================================
    {
        "https://github.deshaw.com/shrimali/desflow.nvim",
        name = "desflow",
        lazy = true,
        cmd = {
            "DESFlow",
            ...
            "DESFlowPreset",
            "DESFlowLogTime",
            "DESFlowRestartBridge",
        },
        keys = {
            { "<leader>df", function() require("desflow").open() end,                 desc = "DESFlow: open" },
            {
                "<leader>dg",
                function()
                    require("desflow.graph").open(require("desflow.panel").get_requests(),
                        require("desflow").get_config(), require("desflow")._open_request)
                end,
                desc = "DESFlow: graph"
            },
            { "<leader>dn", function() require("desflow.panel")._nl_search() end,     desc = "DESFlow: NL search" },
            { "<leader>dO", function() require("desflow.panel")._pick_sort() end,     desc = "DESFlow: sort order" },
            { "<leader>dF", function() require("desflow.panel")._select_preset() end, desc = "DESFlow: preset filter" },
        },
        config = function()
            require("desflow").setup {

                python_path = "python3",
                auto_refresh = false,
                auto_refresh_interval = 60,
                max_results = 50,
                -- sidebar_width is the starting width; press > / < in the panel to resize live.
                sidebar_width = 110,
                -- Columns shown in the tabular request list (panel and graph view).
                -- Each entry can be a string or { key = "status", width = 12 } to override width.
                -- Available keys: "status", "assignee", "reviewer", "logger", "category",
                --                 "priority", "logged_date", "last_updated", "due_date",
                --                 "logged_date_rel", "last_updated_rel", "due_date_rel"
                -- Multi-value fields (assignee, reviewer, logger) expand into extra rows.
                list_columns = {
                    "status", -- 14 cols
                    "assignee", -- 12 cols
                    "reviewer", -- 12 cols
                    "logger", -- 10 cols
                    "category", -- 10 cols
                    "logged_date_rel", --  8 cols  ("1mo ago")
                    "last_updated_rel", --  8 cols
                },
                defaults = {
                    business_area = "GAITech",
                    assignee      = "shrimali",
    ...
    {
        dir = "/codemill/shrimali-r/markdown-server.nvim",
        ft = "markdown",
        keys = {
