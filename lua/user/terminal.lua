...
local function enable_vi_mode(job_id, shell)
  if not config.vi_mode or not job_id then return end
...
function M.node_terminal() M.float_terminal "node" end

function M.ncdu_terminal() M.float_terminal "ncdu" end

function M.htop_terminal() M.float_terminal "htop" end

function M.make_terminal() M.float_terminal "./.buildme.sh" end

function M.cargo_run() M.float_terminal "cargo run" end

function M.cargo_test() M.float_terminal "cargo test" end
...
M.lazygit_toggle = function() M.lazygit_float() end
...
vim.api.nvim_create_autocmd("WinClosed", {
  ...
  callback = function()
    ...
    if terminals.horizontal and vim.api.nvim_buf_is_valid(terminals.horizontal.buf) then
      ...
      if not found then terminals.horizontal = nil end
    end
    ...
    if terminals.vertical and vim.api.nvim_buf_is_valid(terminals.vertical.buf) then
      ...
      if not found then terminals.vertical = nil end
    end
  end,
})
