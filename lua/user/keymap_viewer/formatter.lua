...
function M.format_for_list(keymap)
  ...
  if cmd and cmd ~= "" and cmd ~= desc then formatted = formatted .. " | " .. cmd end
...
function M.format_for_preview(keymap)
  ...
  if keymap.source_file then
    ...
    if keymap.source_line then table.insert(lines, string.format("  Line: %d", keymap.source_line)) end
  else
...
function M.get_searchable_text(keymap)
  ...
  -- Add description
  if keymap.desc then table.insert(parts, keymap.desc) end

  -- Add command
  if keymap.rhs then
    table.insert(parts, keymap.rhs)
    local cmd = utils.extract_command(keymap.rhs)
    if cmd then table.insert(parts, cmd) end
  end

  -- Add source file name
  if keymap.source_file then
    local filename = keymap.source_file:match "([^/]+)$"
    if filename then table.insert(parts, filename) end
  end
