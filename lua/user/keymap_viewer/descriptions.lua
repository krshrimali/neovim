...
function M.generate_from_command(keymap)
  ...
  local ok, result = pcall(function()
    ...
    if not module_path then
      ...
      if not module_path then module_path = rhs:match "require%('([^']+)'" end
    end
...
function M.enrich_descriptions(keymaps)
  for _, km in ipairs(keymaps) do
    if not km.desc or km.desc == "" then
      local ok, desc = pcall(function() return M.generate_from_context(km) end)
      if ok and desc then km.desc = desc end
    end
  end
  return keymaps
end
