  ...
  for _, file_path in ipairs(files) do
    ...
    if ok and content then
      ...
      for i, line in ipairs(lines) do
        ...
        local keymap_set_with_desc = line:match('vim%.keymap%.set%s*%(%s*["\']([^"\']+)["\']%s*,%s*["\']([^"\']+)["\']%s*,%s*[^,]+desc%s*=%s*["\']([^"\']+)["\']')
...
function M.collect_all()
  ...
  for _, km in ipairs(api_keymaps) do
    ...
    if not keymap_map[key] or not keymap_map[key].desc then
  ...
  for _, km in ipairs(file_keymaps) do
    ...
    if keymap_map[key] then
      ...
      if not keymap_map[key].desc and km.desc then
        keymap_map[key].desc = km.desc
      end
