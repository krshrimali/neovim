...
function M.filter(keymaps, query, options)
  ...
  for _, km in ipairs(keymaps) do
    -- Mode filter
    if options.mode and km.mode ~= options.mode then
      goto continue
    end
    
    -- Has description filter
    if options.has_description ~= nil then
      if options.has_description and (not km.desc or km.desc == "") then
        goto continue
      end
      if not options.has_description and km.desc and km.desc ~= "" then
        goto continue
      end
    end
    ...
    if options.buffer_only ~= nil then
      if options.buffer_only and not km.buffer then
        goto continue
      end
      if not options.buffer_only and km.buffer then
        goto continue
      end
    end
    ...
    if options.prefix then
      if not km.key:match("^" .. vim.pesc(options.prefix)) then
        goto continue
      end
    end
    ...
    if normalized_query ~= "" then
      ...
      if not utils.fuzzy_match(searchable, query) then
        goto continue
      end
    end
