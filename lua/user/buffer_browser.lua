...
M.open_buffer_browser = function()
  ...
  if #buffers > 0 then vim.api.nvim_win_set_cursor(win, { 3, 0 }) end
end
...
M.refresh_sidebar = function()
  if
    not M._sidebar_state.is_open
    or not M._sidebar_state.win
    or not vim.api.nvim_win_is_valid(M._sidebar_state.win)
  then
    return
  end
...
M.toggle_sidebar = function()
  ...
  -- Refresh with r
  vim.keymap.set("n", "r", function() M.refresh_sidebar() end, opts)
  ...
  -- Position cursor on first buffer
  if #buffers > 0 then vim.api.nvim_win_set_cursor(win, { 6, 0 }) end
