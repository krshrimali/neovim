...
local function find_glow_path()
  ...
  for _, path in ipairs(paths) do
    if path == "glow" then
      ...
      if vim.v.shell_error == 0 then return vim.trim(result) end
    else
  ...
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
      ...
      vim.keymap.set(
        "n",
        "<leader>mp",
        function() M.preview_floating() end,
        vim.tbl_extend("force", opts, { desc = "Markdown Preview (Float)" })
      )
      ...
      vim.keymap.set(
        "n",
        "<leader>mv",
        function() M.preview_vertical_split() end,
        vim.tbl_extend("force", opts, { desc = "Markdown Preview (Vertical Split)" })
      )
      ...
      vim.keymap.set(
        "n",
        "<leader>mr",
        function() M.refresh_preview() end,
        vim.tbl_extend("force", opts, { desc = "Refresh Markdown Preview" })
      )

      -- Auto-refresh on save
      vim.api.nvim_create_autocmd("BufWritePost", {
        buffer = 0,
        callback = function() M.refresh_preview() end,
      })
    end,
  })
end
