...
-- Utility: escape Lua patterns (for Neovim <0.8)
local function lua_pattern_escape(str) return str:gsub("(%W)", "%%%1") end

-- Use correct escaping function for Lua patterns
local function lua_pesc(s)
  if vim.pesc then
    return vim.pesc(s)
  else
    return lua_pattern_escape(s)
  end
end
...
local function get_repo_relative_path(path)
  ...
  if abs_path == git_root then return "." end
  local rel = abs_path:gsub("^" .. lua_pesc(git_root) .. "/", "")
  return rel
end

local dir_cache = {}

local function clear_cache() dir_cache = {} end
...
local function scan_directory(path, level)
  ...
  while true do
    ...
    if not config.show_hidden and name:match "^%." then goto continue end
...
local function flatten_tree(items, result)
  ...
  for _, item in ipairs(items) do
    ...
    if item.expanded and #item.children > 0 then flatten_tree(item.children, result) end
  end
  return result
end
...
local function render_tree()
  ...
  if current_root ~= parent_path and current_root ~= "/" then table.insert(lines, ".. ") end
...
local function get_item_at_cursor()
  ...
  if adjusted_line < 1 or adjusted_line > #flat_items then return nil end
...
local function open_file(path, split_type, should_close_tree)
  ...
  if not is_absolute_path(path) then path = current_root .. "/" .. path end
  ...
  if should_close_tree then M.close() end
end
...
local function get_relative_path(path)
  ...
  path = path:gsub("\\", "/")
  local escaped_cwd = lua_pesc(cwd)
  if path == cwd then return "." end
  if path:find("^" .. escaped_cwd) then
...
-- Safe os.execute (LuaJIT/Lua 5.2+/Lua 5.1)
local function safe_os_execute(cmd)
  local result1, result2, code = os.execute(cmd)
...
local function open_github_link(path)
  ...
  if vim.fn.has "mac" == 1 then
    safe_os_execute(string.format("open '%s'", github_url))
  elseif vim.fn.has "win32" == 1 then
    safe_os_execute(string.format('cmd /c start "" "%s"', github_url))
  else
    safe_os_execute(string.format("xdg-open '%s'", github_url))
  end
...
local function navigate_to_parent()
  ...
  if current_root ~= parent_check_path and current_root ~= "/" then cursor_line = 2 end
  if tree_win and vim.api.nvim_win_is_valid(tree_win) then vim.api.nvim_win_set_cursor(tree_win, { cursor_line, 0 }) end
end
...
local function delete_item()
  ...
  if item.type == "directory" then
    if vim.fn.delete(item.path, "rf") == 0 then
      ...
      success = safe_os_execute("rm -rf '" .. item.path .. "'")
    end
  else
...
local function rename_item()
  ...
  if not success then success = os.rename(item.path, new_path) end
  if success then
...
local function handle_enter()
  ...
  if item.type == "directory" then
    ...
    open_file(item.path, "default", false)
  end
end
    ...
    actions = {
      ["enter"] = function(selected)
        if selected and #selected > 0 then
          ...
          if not is_absolute_path(sel) then sel = current_root .. "/" .. sel end
...
local function show_context_menu()
  ...
  local function close_menu()
    if vim.api.nvim_win_is_valid(menu_win) then vim.api.nvim_win_close(menu_win, true) end
  end
  ...
  local function move_to_next_valid_item(direction)
    ...
    repeat
      ...
      if next_idx == current_idx then break end
...
function M.open(root_path)
  ...
  vim.keymap.set("n", "u", function() navigate_to_parent() end, opts)
  ...
  if vim.o.mouse ~= "" then
    vim.keymap.set("n", "<LeftMouse>", handle_enter, opts)
    vim.keymap.set("n", "<RightMouse>", show_context_menu, opts)
  end
  ...
  if #tree_data > 0 then vim.api.nvim_win_set_cursor(tree_win, { 1, 0 }) end
end
...
function M.open_workspace() M.toggle(vim.fn.getcwd()) end
